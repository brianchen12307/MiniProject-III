...
public:
    ...
    int cur_player, eat_discs, place_advantage, state_value;
private:
...
public:
    ...
    void put_disc(Point p) {
        ...
        state_value = curValue();
    }

    int curValue() {
        float value;
        float enmey_valid = next_valid_spots.size();
        float enmey_valid_sum = 0, enmey_avg = 0;

        for (Point it : next_valid_spots) {
            enmey_valid_sum += map[it.x][it.y];
        }
        if (!enmey_valid)
            enmey_avg = enmey_valid_sum / next_valid_spots.size();

        
        value = (place_advantage - enmey_avg) * 3 - enmey_valid * 8 + eat_discs;
...
Point minmax(State board, int depth) {
    if (depth == AIdepth || board.next_valid_spots.empty()) {
        Point tmp;
        tmp.state_value = board.state_value;
        return tmp;
    }
    else {
...
Point alphabeta(State state, Point a, Point b, int depth) {
    if (depth == AIdepth || state.next_valid_spots.empty()) {
        Point tmp(0, 0, state.state_value);
        return tmp;
    }
    else {
